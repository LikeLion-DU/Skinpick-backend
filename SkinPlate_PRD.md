...
| S07 | Skin Plate 결과 | Plate Score, 좋은 점 / 주의사항 / **추천 행동** 카드, **"왜 65점인가" 접이식 계산 내역**, **행동 실행 버튼(점수 재계산 애니메이션)** | → S08 |
...
| `skin_plate.applied_rules` | 어떤 룰이 점수에 기여했는지 저장 → "왜 97점인가"를 사후 설명 가능 |
...
| G3 | Day 7 종료 | Plate Score E2E 동작 | **룰은 자르지 않는다.** 9종은 각각 20줄이고 이미 다 작성돼 있어 잘라도 아끼는 시간이 없는데, 발표 숫자(65점/97점)만 깨진다. 대신 화면을 자른다 — 결과 공유 · 지표 추이 차트 |
...
| 1:40~1:50 | **"왜 65점인가" 카드 펼치기** → 기본 75 · 단백질 +7 · 발효식품 +5 · 나트륨 −8 · 포화지방 −2 · 매운맛×홍조 −12 |
...
| **S07 계산 내역 카드** | "왜 65점인가"를 펼쳐 보여준다 — 차별점을 문서에서 화면으로 |
