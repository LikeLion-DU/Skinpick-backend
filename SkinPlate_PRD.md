...
│   │   │   ├── StandardFoodTable.java          # 공공데이터 1,532종 조회
...
| R18 | **AI 영양 추정치 변동으로 점수가 흔들림** | 재현성 주장 붕괴 | 높 | 음식명 매칭으로 공공데이터 표준값(식약처 1,532종)을 덮어쓴다. 화면에 "표준 영양 DB 기준" 라벨 표기 |
...
| "같은 사진 두 번 찍으면 같은 점수 나옵니까?" | 룰 엔진은 결정론적이다. 다만 영양값 추정은 AI가 하므로 흔들릴 수 있어, **음식명이 잡히면 식약처 공공데이터 1,532종의 표준값을 사용**한다(화면에 라벨 표기). AI는 "무슨 음식인가"만 판단한다 |
...
| 재현성 | 음식명이 잡히면 공공데이터 표준값(1,532종)으로 덮어쓴다. AI는 "무슨 음식인가"만 판단 |
