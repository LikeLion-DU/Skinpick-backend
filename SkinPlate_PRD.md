...
| 서버 **피부 28초 · 리포트 문장 12초 · 나머지 25초** · 클라이언트 32초 | **리포트만 기준이 다르다** — 32초 예산이 아니라 "조회 화면이 그만큼 멎어도 되는가"다. 주간 문장은 실패를 삼키므로(점수·영양·고민은 그대로 나간다) 짧게 끊어도 잃는 것이 문장 하나뿐이고, 나머지 넷은 실패하면 기능 자체가 없어 같은 선택을 못 한다. 코드가 강제하는 리포트 상한은 15초다. 피부는 `detail:"high"` 세 장이라 18초로는 빠듯하다. 세 장을 한 요청에 넣은 대가는 지연이지, 호출 횟수가 아니다. 28 은 코드가 강제하는 상한이다 — 재시도 대기(2초)를 더한 전체 마감이 클라이언트 32초 안에 들어와야 한다 |
...
| 상황 | 처리 |
|---|---|
| 타임아웃 (피부 28초 · 리포트 문장 12초 · 나머지 25초 초과) | `AI_TIMEOUT`(504) 반환, 앱은 재시도 버튼 노출. **리포트만 예외** — 서비스가 삼켜서 `aiComment` 키만 빠지고 200 이 나간다 |
...
>
> **타임아웃은 12초로 따로 둔다**(`app.ai.report-timeout-seconds` · 재시도 대기까지 포함한 전체 마감 14초 · 코드가 강제하는 상한 15초). 리포트는 하루에도 여러 번 여는 조회 화면이라 분석용 25초를 물려받으면 캐시가 빈 첫 조회에서 화면이 통째로 멎는다. 가르는 기준은 **"실패해도 화면이 남는가"** 다 — 주간 문장은 실패를 삼켜 점수·영양·고민이 그대로 나가지만, 나머지 네 호출은 실패하면 기능 자체가 없어 같은 선택을 못 한다.
>
...
app:
  ...
  ai:
    ...
    report-timeout-seconds: ${REPORT_TIMEOUT_SECONDS:12}  # 주간 문장 전용. 상한 15 — 조회 화면이 멎는 시간이다
    mock: ${AI_MOCK:false}
