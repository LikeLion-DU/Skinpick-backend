...
| 서버 **피부 28초 · 나머지 25초** · 클라이언트 32초 | 피부는 `detail:"high"` 세 장이라 18초로는 빠듯하다. 세 장을 한 요청에 넣은 대가는 지연이지, 호출 횟수가 아니다. 28 은 코드가 강제하는 상한이다 — 재시도 대기(2초)를 더한 전체 마감이 클라이언트 32초 안에 들어와야 한다 |
...
| 상황 | 처리 |
|---|---|
| 타임아웃 (피부 28초 · 나머지 25초 초과) | `AI_TIMEOUT`(504) 반환, 앱은 재시도 버튼 노출 |
...
>
> **타임아웃은 8초로 따로 둔다**(`app.ai.report-timeout-seconds` · 재시도 대기까지 포함한 전체 마감 10초). 분석은 사용자가 "분석 중"을 보며 기다리는 화면이지만 리포트는 하루에도 여러 번 여는 조회 화면이라, 분석용 25초를 물려받으면 캐시가 빈 첫 조회에서 화면이 통째로 멎는다. 짧게 끊어도 잃는 것이 문장 하나뿐이라 가능한 선택이다 — **점수·영양·고민은 그대로 나간다.**
