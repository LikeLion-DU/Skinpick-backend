...
| S07 | Skin Plate 결과 | Plate Score, 좋은 점 / 주의사항 / **추천 행동** 카드, **"왜 58점인가" 접이식 계산 내역**, **행동 실행 버튼(점수 재계산 애니메이션)** | → S08 |
...
6. **S07에서 점수가 눈앞에서 움직여야 한다.** "국물을 절반만 남기면 점수가 상승합니다"라는 *문장*과, 버튼을 눌러 `58 → 66`이 실제로 오르는 *경험*은 다른 제품이다. 이 5초가 이 서비스의 전부다.
...
| `API_BASE_URL` | `http://10.0.2.2:8080` | `https://1-201-116-157.sslip.io` (2026-08-18 배포 완료) |
| 앱 빌드 | 디버그 | **릴리즈** |
...
| 순서 | 할 일 | 빠뜨리면 | 2026-08-18 |
|---|---|---|---|
| 1 | `api.<도메인>` **A 레코드 → 가비아 공인 IP** | Let's Encrypt 발급이 실패하고 Caddy 가 재시도만 반복한다. 로그를 안 보면 "왜 https 가 안 되지"로 끝난다 | ✅ sslip.io 로 대체 (아래) |
| 2 | 방화벽 **80·443 개방** | 80 이 막히면 HTTP-01 챌린지가 통과하지 못한다. 443 만 열어도 발급이 안 된다 | ✅ 22·80·443 |
| 3 | 앱을 **`-p 127.0.0.1:8080:8080`** 으로 띄운다 | `0.0.0.0` 이면 HTTPS 를 세워 놓고도 **평문 8080 이 인터넷에 그대로 남는다.** 테스트 계정이 켜져 있으면 그 포트로 누구나 토큰을 받는다 | ✅ 외부에서 8080 닫힘 확인 |
| 4 | Caddy 기동 후 **3장 업로드를 한 번 통과**시킨다 | 413·502 는 배포 직후가 아니라 심사 중에 처음 만나게 된다 | ⚠️ 14MB 통과 확인 · **실사진 미검증** |
...
#### 실배포 기록 — **2026-08-18 완료**

**배포 URL 은 `https://1-201-116-157.sslip.io` 다.** 도메인을 확보하지 못해 **sslip.io** 로 갔다 — IP 를 그대로 이름으로 되돌려주는 공개 DNS 라 도메인 구매도 A 레코드 설정도 없이 위 배포 순서 1번을 건너뛴다. 발급되는 인증서는 자체 서명이 아니라 **Let's Encrypt 정식 인증서**다. sslip.io 가 Public Suffix List 에 올라 있어 **발급 한도가 다른 sslip.io 사용자와 섞이지 않는 것**도 이 선택을 안전하게 만든다. 실제 도메인이 생기면 `/etc/caddy/Caddyfile` 한 줄을 바꾸고 `docker restart caddy` 면 되므로, 지금 한 작업 중 버려지는 것은 없다.

| 항목 | 결과 |
|---|---|
| 서버 | 가비아 VM · Ubuntu 22.04.5 LTS · 2 vCore · 4GB · 루트 100GB (데이터 스토리지 없음) |
| 인증서 | Let's Encrypt · `1-201-116-157.sslip.io` · ~2026-11-16 |
| HTTP → HTTPS | 308 리다이렉트 |
| Flyway | V4~V8 적용, 현재 v8 · `ddl-auto: validate` 통과 |
| 업로드 | **14,156,270 bytes 가 Caddy·multipart 를 통과**해 `ImageEncoder` 의 400 까지 도달 (0.53초) |
| 시연 로그인 | `POST /api/v1/auth/test-login` 200 · 7일 토큰 |

> **Caddy 가 실제로 쓴 챌린지는 HTTP-01 이 아니라 TLS-ALPN-01(443) 이었다.** 그래도 80 은 열어 둔다 — 발급 경로는 Caddy 가 상황에 따라 고르는 것이라 다음에도 443 쪽이 된다는 보장이 없고, 80 이 닫혀 있으면 HTTP→HTTPS 리다이렉트도 같이 죽는다.

> **위 업로드 검증은 더미 바이트다.** Caddy → multipart → 컨트롤러까지만 증명했고 **OpenAI Vision 왕복은 아직 한 번도 안 돌았다.** 배포 순서 4번은 실사진 3장으로 다시 통과시켜야 닫힌다(G5).

> **재배포는 서버의 `~/skinplate/deploy.sh` 한 줄이다.** `git pull` → 빌드 → 교체 → 헬스체크 대기까지 한다. GitHub Actions 자동 배포는 **의도적으로 만들지 않았다** — 남은 기간의 재배포 10~15회가 아끼는 시간(20~30분)보다 파이프라인 셋업(1~2시간)이 비싸고, 그 시간이 QA 에서 나온다.
...
| 배포 (시연) | `prod` + `TEST_ACCOUNT_ENABLED=true` | **활성** | `https://1-201-116-157.sslip.io/api/v1` · **APK·웹 빌드 모두 이 값** · DB는 Supabase |
| 배포 (실서비스) | `prod` | 비활성 | 〃 |
...
| 시점 | 할 일 |
|---|---|
| ~~Day 4~~ **2026-08-18 완료** | **1차 배포.** 기능이 절반만 돌아도 올린다. 목적은 파이프라인을 뚫는 것. **가비아 VM + 리버스 프록시(HTTPS·본문 20MB)를 같이 세운다**(R23) → ✅ 위 실배포 기록 |
...
│   │   │   ├── StandardFoodTable.java          # 공공데이터 1,443종 조회
...
erDiagram
    ...
    FOOD_ANALYSIS {
        ...
        int sodium_mg
        numeric sugar_g
        numeric saturated_fat_g "V9 · R11 이 총지방 대신 본다"
        numeric fiber_g "V9 · R15 가 100kcal 당 밀도로 본다"
        int vitamin_a_ug "V9 · R06 이 태그와 OR 로 본다"
        numeric vitamin_c_mg "V9 · 같음"
...
| `skin_plate.applied_rules` | 어떤 룰이 점수에 기여했는지 저장 → "왜 93점인가"를 사후 설명 가능 |
...
| `REMOVE_BATTER` | 튀김옷 일부 제거 | `cookingMethod = GRILLED` (R07 해제), **관찰 기름기(oiliness) HIGH → MEDIUM** — HIGH 로 남기면 R07 이 기름기 경로로 다시 걸려 버튼 효과가 사라진다. **지방은 손대지 않는다** — 2026-08-18 부터 R11 이 포화지방을 보지만, 튀김옷을 걷어낸 뒤의 포화지방을 이 앱이 알 방법이 없다. 없는 숫자를 지어내는 것보다 안 건드리는 편이 설명 가능하다 |
| `LESS_RICE` | 밥·면 양을 조금 줄인다 | `caloriesKcal × 0.75`. **R10 이 완전히 꺼지는 것은 880kcal 까지**다(×0.75 = 660). 그 위는 단계만 내려가고, 1,053kcal 을 넘으면 2단계에 그대로 남아 회복이 0 이라 행동 카드가 아예 안 붙는다. 탄수는 손대지 않는다 — R12 는 정제 탄수 **재료가 있는가**를 함께 묻는 룰이라 밥을 덜 먹었다고 재료가 사라지지 않는다 |
...
> **관리 엔티티를 절대 만지지 않는다.** `plate.getFoodAnalysis()`를 꺼내 그 자리에서 고치면, `LESS_SPICY`가 `orphanRemoval = true` 컬렉션에서 CAPSAICIN을 지우는 순간 **DB에서 재료 행이 삭제**되고 `HALVE_SOUP`은 `food_analysis.sodium_mg`를 영구히 925로 바꾼다. 무대에서 버튼을 누르면 58 → 70이 뜨고, 뒤로 갔다 다시 들어오면 원래 점수가 70이다. 구현 세부는 **DTO 설계서 §1.19.2** 참조.

> **이 API가 이 프로젝트에서 가장 값싼 차별화다.** 작업량은 반나절인데, 심사위원 눈앞에서 `58 → 66`이 움직인다. 다른 팀의 "AI가 조언을 해줍니다"와 우리의 "조언을 실행하면 점수가 이만큼 오릅니다"는 완전히 다른 인상을 남긴다.
>
> 이전 설계의 `potentialScore`(감점 절댓값 합산)는 **어떤 실제 계산과도 일치하지 않았다.** 예시 A에서 58 + 8 + 6 = 72가 나오는데, 실제로 국물을 절반 남기면 나트륨이 925mg이 되어 R04가 아예 발동하지 않으므로 **66**이고, 매운 양념까지 덜면 **78**이다. 72는 어디에도 없는 숫자다. 근사치를 버리고 실제로 다시 계산한다.
...
| **R10** | 열량 > 660 / 790kcal | **−4 / −7** | CAUTION | 열량이 높음 | 밥·면 줄이기 (계산) |
| **R11** | **포화지방** > 3.9 / 8.2 / 12.3g | **−2 / −7 / −10** | CAUTION | 포화지방 많음 | — |
| **R12** | HIGH_GI 태그 **그리고** 탄수 ≥ 30g | **−4** | CAUTION | 정제 탄수 많음 | — |

**피부 축** — 현재 지표로만 발동한다. 제품이 존재하는 이유. 실측 범위 −15 ~ +23.

| 코드 | 조건 (피부 × 음식) | Δ | 타입 | 메시지 | 추천 행동 |
|---|---|---|---|---|---|
| **R01** | 건조(hydration<40) × 수분/오메가3 재료 | **+8** × 심각도 | GOOD | 수분 보충 재료 | — |
| **R08** | 장벽 약화(barrier<40) × OMEGA3 | **+7** × 심각도 | GOOD | 오메가3 함유 | — |
| **R02** | 홍조(redness>60) × 매운 음식/CAPSAICIN | **−10** × 심각도 × 강도 | CAUTION | 매운맛 자극 | 매운 양념 덜기 (계산) |

> **심각도 3단계 (2026-08-18)** — `SeverityCalculator` 가 1.5(≥80) · 1.2(≥60) · **0.6(그 미만)** 을 낸다.
> 마지막 값이 이번 개편의 핵심이다. 예전에는 R03·R07 이 피부 게이트 뒤에 있어 트러블·유분이 정상이면
> **발동 자체를 안 했고**, 그래서 감자튀김과 닭가슴살 샐러드가 똑같이 70점이었다. 게이트를 떼고
> 개인화를 "발동 여부" 에서 "크기" 로 옮긴다 — 튀김 −10 기준 정상 −6 · 중간 −12 · 심함 −15.
> 게이트가 남은 R01·R02·R08 은 임계값이 전부 심각도 60 이상이라 이 구간에 닿지 않는다(점수 무변화).
>
> **회복치는 상수가 아니다 — 다섯 카드 전부.** R03·R04·R10 은 행동 이후의 단계를 다시 세어 그 차이를
> 싣는다. 옛 고정값은 나트륨 1,200mg 국에 "+8" 이라 말하고 4 만 올렸고, 1,200kcal 한 끼에 "+5" 라
> 말하고 0 이 올랐다. 회복이 0 이면 행동 카드를 아예 주지 않는다.
> R02·R07 은 행동이 룰을 통째로 끄므로 **회복치가 곧 그 감점의 절댓값**이다 — 심각도·강도 계수가
> 이미 곱해진 값이라 상수로 광고할 수 없다. 홍조 64 에서 매운맛 카드는 "+6" 이라 적고 12 를 올렸고,
> 유분이 심한 사용자의 튀김옷 카드는 "+5" 라 적고 15 를 올렸다.
>
> **가점만 밀도로 잰다.** R15·R06 은 100kcal 당 함량을 본다. 절대량이면 감자튀김(식이섬유 5.8g)이
> 샐러드(3.8g)를 이긴다 — 감자튀김이 더 크기 때문이고, 그러면 점수가 음식의 질이 아니라 크기를 잰다.
> 감점은 절대량 그대로다. 부담은 실제로 먹은 총량이 만든다.
>
> **표준 영양 확장 (V9)** — 원본 160 컬럼 중 여섯 개만 쓰고 있었다. 포화지방·식이섬유·비타민A/C·아연을
> 더했다(요리 2,589행 기준 결측률 0 · 6 · 11 · 6 · 28%). 비타민E·오메가3 지방산(46%)·트랜스지방(77%)·
> 셀레늄(82%)은 결측이 많아 뺐다 — "없어서 안 걸림" 과 "적어서 안 걸림" 이 구분되지 않는다.
> **아연은 저장만 하고 점수 룰로 쓰지 않는다.** 아연 밀도가 단백질 밀도와 r=0.38 로 겹치고(식이섬유는
> 0.10), 상위가 굴·조개·갈비탕·수육·스테이크라 부담이 큰 음식에 가점을 주게 된다. 일일·주간 리포트의
> 영양소 지표로 쓸 자리가 있어 컬럼은 남긴다.
>
...
> **같은 사람, 다른 한 끼 — 58점과 93점.** 이 35점 차이가 제품이 존재하는 이유다. 두 음식 모두
> 나트륨은 높지만, 오늘의 피부가 건조하고 홍조가 있기 때문에 매운 찌개는 더 크게 감점되고
> 오메가3는 더 크게 가점된다.
>
> **R14 는 R01 과 독립이다.** 건조한 사용자의 연어가 두 룰을 다 받는 것은 이중계상이 아니라
> "이 사람에게 가장 필요한 음식" 이라는 뜻이다. 총지방으로 지방을 재던 시절에는 OMEGA3 태그가
> 감점 완화의 대리 지표로도 쓰여 상호배제가 필요했지만, R11 이 실측 포화지방으로 바뀌며 사라졌다.

#### 18.7.1 표준 음식 1,443종 전수 검증 (2026-08-18)

`StandardFoodScoringTest` 가 표준 음식 전체를 실제 엔진에 통과시켜 이 표를 고정한다.
단위 테스트는 룰 하나가 제 값을 내는지만 보는데, 이 앱이 실패한 방식은 그게 아니라
**"룰은 전부 맞는데 감자튀김과 닭가슴살 샐러드가 같은 70점"** 이었다.

| 피부 프로필 | 평균 | σ | min~max | 70점 | ≤40 | 41~60 | 61~80 | 81+ |
|---|---|---|---|---|---|---|---|---|
| 정상 `60/50/40/40/60` | 71.1 | **8.3** | 37~87 | 18% | 0% | 9% | 77% | **13%** |
| 중간 `45/65/65/65/45` | 68.7 | 10.6 | 20~85 | 15% | 2% | 17% | 68% | 12% |
| 심함 `35/85/85/85/35` | 68.6 | 12.7 | 12~100 | 16% | 4% | 20% | 60% | 16% |
| 시연 `38/52/64/25/78` | 70.3 | 9.6 | 29~95 | 16% | 1% | 12% | 72% | 15% |

개편 전에는 정상 피부에서 σ 3.7 · 70점 50% · **81점 이상 0%** 였다.

**대표 음식 (정상 피부)** — 건강식 최저 74 > 부담식 최고 66.

```
된장국_시금치 83  참치구이 82  김밥 75  닭가슴살 샐러드 75  콩나물무침 74  고등어구이 74
초밥 70  샐러드 68
떡볶이 66  치즈볼 61  감자튀김 58  돈가스_치즈 56  스테이크 55  탄탄면 52
```

**개인화 (같은 음식, 정상 → 심함)**

```
고등어구이  74 → 92   된장국_시금치 83 → 93
떡볶이     66 → 51   매운탕 73 → 58   감자튀김 58 → 49
```

### 18.8 룰 구현 예시

> *(아래 두 블록은 `PlateRule` 이 어떻게 생겼는지 보여주는 **골격 예시**이고, 숫자는 2026-08-18 재캘리브레이션 이전 것이다 — 나트륨은 초과량 비례가 아니라 3단계 계단(1150/1700/2300)이고 회복치도 상수가 아니라 단계 변화로 계산한다. **최종 룰표는 §18.6, 실제 코드는 `plate/engine/rules/` 를 본다.**)*
...
| G3 | Day 7 종료 | Plate Score E2E 동작 | **룰은 자르지 않는다.** 9종은 각각 20줄이고 이미 다 작성돼 있어 잘라도 아끼는 시간이 없는데, 발표 숫자(58점/93점)만 깨진다. 대신 화면을 자른다 — 결과 공유 · 지표 추이 차트 |
...
| R18 | **AI 영양 추정치 변동으로 점수가 흔들림** | 재현성 주장 붕괴 | 높 | 음식명 매칭으로 공공데이터 표준값(식약처 1,443종)을 덮어쓴다. 화면에 "표준 영양 DB 기준" 라벨 표기 |
...
| R23 | ~~VM 에는 HTTPS 가 자동으로 안 붙는다~~ | — | — | **해소(2026-08-18).** Caddy + sslip.io 로 Let's Encrypt 정식 인증서 발급, HTTP→HTTPS 308 확인(§9.6 실배포 기록) |
| **R24** | **리버스 프록시 본문 상한**(nginx 기본 1MB) | **15MB 짜리 3장 업로드가 전부 413.** 앱에는 "분석 실패"만 뜨고 백엔드 로그에는 요청이 아예 안 찍힌다 | 낮 | **대부분 해소(2026-08-18)** — Caddy 로 **14,156,270 bytes 통과 확인.** 다만 더미 바이트라 **실사진 3장 왕복은 미검증**이다. 완전히 닫히는 것은 G5(§9.6 실배포 기록) |
...
| 1:00~1:40 | **김치찌개 촬영** → Plate Score 58, "나트륨 과다·포화지방·매운맛 자극" |
| 1:40~1:50 | **"왜 58점인가" 카드 펼치기** → 기본 70 · 단백질 +6 · 발효식품 +4 · 나트륨 −8 · 포화지방 −2 · 매운맛×홍조 −12 |
| 1:50~2:00 | **[국물 절반 남기기] 버튼** → **58 → 66 애니메이션** ★클라이맥스 |
...
| "같은 사진 두 번 찍으면 같은 점수 나옵니까?" | **표준 음식표에서 찾히면 같은 점수가 나온다.** 2026-08-18 부터 영양값뿐 아니라 조리법·매운맛·재료 태그·강도 계수까지 표준표가 확정한다 — AI 가 회차마다 다른 태그를 줘도 점수가 움직이지 않는다(같은 떡볶이 사진 4회 반복 실측). AI 는 "무슨 음식인가"만 판단하고, 표에서 못 찾은 음식만 AI 추정 영양값에 기댄다 |
...
| 재현성 | 음식명이 잡히면 공공데이터 표준값(1,443종)으로 덮어쓴다. AI는 "무슨 음식인가"만 판단 |
...
| **⑦-b `POST /plates/{id}/simulate`** | 추천 행동 실행 시 점수 재계산. `58 → 66`이 화면에서 움직인다 |
| **S07 계산 내역 카드** | "왜 58점인가"를 펼쳐 보여준다 — 차별점을 문서에서 화면으로 |
