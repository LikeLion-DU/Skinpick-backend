...
|---|---|
| 문서 버전 | v1.9 |
| 작성일 | 2026-08-07 · **v1.9 개정 2026-08-17** (일일 리포트 신규 · 주간 리포트를 일일 집계로 개편) |
...
| 얼굴 크기 | 바운딩 박스 높이 ≥ 프레임 높이의 34% | "조금 더 가까이 와주세요" |
...
> **얼굴 크기 34% 는 실기기 재교정 값이다(2026-08-16).** 처음 40% 로 잡았는데, 팔을 자연스럽게
> 뻗어 정면을 잡으면 갤럭시 S25 에서 38.5% 가 나와 "조금 더 가까이"에서 촬영이 시작조차 되지
> 않았다. 화면 가이드 타원이 프레임 높이의 45% 이고 ML Kit 박스는 그 안쪽 75~85% 라, 타원을
> 채운 얼굴이 34~38% 로 잡히는 것이 정상이다. **40% 는 타원을 넘겨야 통과하는 값이었다.**
> 해상도는 크롭이 받아낸다 — 업로드하는 것은 얼굴 박스에 여백을 붙여 잘라낸 이미지다.
...
│   │   │   ├── StandardFoodTable.java          # 공공데이터 1,452종 조회
│   │   │   └── StandardFood.java               # 표준 음식 한 건(영양값·조리법·매운맛·태그)
│   │   ├── repository/
...
| **429는 재시도한다** | 예산이 남는 것과 초당 처리량 상한(TPM)은 다른 축이다. `detail:"high"` 세 장으로 요청당 토큰이 커진 만큼 상한에 더 빨리 닿는다. 팀 4명이 동시에 개발하는 Day 3~5, 표준 10종을 반복 호출하는 Day 8 캘리브레이션이 위험 구간이다. **재시도까지 포함한 전체 마감을 코드가 걸어 둔다** — `타임아웃 + 재시도 대기 2초` 를 넘기지 않으므로 429 가 언제 오든 클라이언트 32초 안에 들어온다. ("429 는 즉시 온다"는 예전 가정이었고, 혼잡할 때는 보장이 아니었다.) 재시도가 유일한 정답인 에러를 타임아웃과 같이 묶어 없애면, 예산이 84% 남은 채로 "분석에 실패했습니다"가 뜬다 |
| 서버 **피부 28초 · 조회 문장 12초 · 나머지 25초** · 클라이언트 32초 | **조회 경로만 기준이 다르다** — 32초 예산이 아니라 "화면이 그만큼 멎어도 되는가"다. 주간 코멘트(`GET /reports/weekly`)와 인사이트(`GET /skin-insights`)는 조회 도중 동기로 생성되므로 그 시간이 그대로 빈 화면이다. 분석 세 경로는 POST 이고 앱이 "분석 중" 단계를 그리며 기다린다. 코드가 강제하는 조회 상한은 15초다. 피부는 `detail:"high"` 세 장이라 18초로는 빠듯하다. 세 장을 한 요청에 넣은 대가는 지연이지, 호출 횟수가 아니다. 28 은 코드가 강제하는 상한이다 — 재시도 대기(2초)를 더한 전체 마감이 클라이언트 32초 안에 들어와야 한다 |
...
| 상황 | 처리 |
|---|---|
| 타임아웃 (피부 28초 · 조회 문장 12초 · 나머지 25초 초과) | `AI_TIMEOUT`(504) 반환, 앱은 재시도 버튼 노출. **리포트만 예외** — 서비스가 삼켜서 `aiComment` 키만 빠지고 200 이 나간다 |
...
> **크레딧은 제약이 아니지만 요청 수는 제약이다.** 피부 분석을 `detail:"high"`로 올리는 결정에 비용 부담이 없다는 것은 여전히 맞다. 다만 아래 §17.2 에서 드러난 대로 **계정 일일 요청 한도(50회)가 먼저 걸린다** — 앱에 제한 로직을 만들 일은 아니고, 계정 티어를 올려서 푼다.
| ~~호출 제한~~ | — | **앱에는 만들지 않는다.** 크레딧은 남는다. 막히는 것은 계정 일일 요청 한도이고, 그건 코드가 아니라 결제 수단으로 푼다(§17.2) |
...
> `snapshot_*` 는 **주제를 고르고 문장을 만든 그 순간**의 값이다. 저장할 때 프로필을 다시 읽으면, 그 사이의 최대 12초짜리 AI 호출 동안 사용자가 `PATCH /auth/me` 로 수면을 "충분해요"로 바꿨을 때 **"수면이 부족하다고 기록되고 있어요" 라는 문장 옆에 `ENOUGH` 가 저장된다.** 1회 고정이라 영구히 그 상태다.

---

### 18.11 리포트(일일 · 주간) 집계 구조 [v1.9]

**주간은 일일만 본다.** 원본 기록을 다시 훑지 않는다.

```
skin_plate(+feedback) ─1회 범위조회→ 날짜별 그룹 ─DailyReportAssembler→ 일일 리포트[]
                                                                          ↓
                                        평균 · BEST/WORST · 영양 평균 · 고민 평균 → 주간
```

**리포트의** 하루 점수 정의(`round(avg(plate_score))`)는 `DailyReportAssembler` 한 곳이다. 일일과 주간이 같은 날을 다르게 세는 경로가 구조적으로 없다. **월간도 기간만 넓혀 같은 코드를 탄다** — `weekly_report` 테이블은 만들지 않는다.

> 히스토리(`GET /plates?from=&to=`)의 `days[].plateScore` 도 같은 산식을 `PlateHistoryService` 가 따로 갖고 있다. 한쪽만 바꾸면 같은 날이 두 화면에서 다른 점수로 뜬다 — 히스토리가 리포트로 대체되면 그 코드와 함께 사라질 중복이다.

**기록이 없는 날은 목록에 없다.** 0점으로 채우지 않는다 — 7일 중 5일 기록이면 분모가 5다. 성실하게 기록할수록 점수가 낮아지는 집계는 리포트가 아니다.
...
>
> 1. **고민은 사용자가 언제든 바꾼다**(`PATCH /auth/me`). 기록 저장 시점에 스냅샷을 뜨면, 나중에 고민을 추가한 사용자는 과거 날짜에 그 고민의 행이 없어 리포트가 뚫린다. 반대로 조회 시점에 뜨면 그건 확정 데이터가 아니라 캐시다. **어느 쪽도 "확정"이 되지 않는다.**
> 2. 확정이 필요한 것은 이미 행으로 확정돼 있다 — `plate_score` · `score_delta` · 영양값. **일일 종합 점수·영양·기록 수는 매핑표를 바꿔도 미동도 하지 않는다.** 움직이는 것은 고민 점수 하나뿐이다.
> 3. 룰 자체(R01~R09)를 추가·삭제할 계획이 08-21 전에 없다. 있다 해도 그때 이 표를 함께 고치는 것이 정상 경로다.
>
> 매핑이 빈 고민(`DARK_CIRCLE`)은 응답에서 통째로 빠진다. 대응하는 식단 룰이 없는데 억지로 나트륨에 붙이면, 이 앱이 증명할 수 없는 인과를 숫자로 말하게 된다.

#### 주간 AI 코멘트 — 같은 집계에는 같은 문장

AI 는 **집계된 표만** 받는다(§18.9 의 경계 그대로 — 판단은 규칙, 문장만 AI). 한 주치 음식 기록 원문을 다시 보내지 않는다.

> **캐시 키는 `(userId, 프롬프트)` 다.** 기록이 하나라도 늘거나 기간이 달라지면 프롬프트가 달라져 새로 생성되고, 같은 집계를 다시 열면 호출이 없다. 시간 기반 TTL 이 필요 없는 이유다 — 무효화 조건이 "데이터가 바뀌었을 때"와 정확히 일치한다.
>
> **`userId` 를 키에서 빼면 안 된다.** 프롬프트에는 개인 식별 정보를 싣지 않으므로(§17.3 의 원칙) 두 사용자의 집계가 우연히 같아질 수 있고, 그때 남의 문장이 그대로 나간다.
>
> 프로세스 안의 LRU 다(테이블·Redis 없음). 재기동하면 비고, 인스턴스가 여러 대면 각자 갖는다 — 현재 배포는 가비아 VM 1대다. **생성 실패는 캐시하지 않는다**(다음 조회에서 다시 시도한다).
>
> **타임아웃은 12초로 따로 둔다**(`app.ai.view-timeout-seconds` · 재시도 대기까지 포함한 전체 마감 14초 · 코드가 강제하는 상한 15초). 리포트는 하루에도 여러 번 여는 조회 화면이라 분석용 25초를 물려받으면 캐시가 빈 첫 조회에서 화면이 통째로 멎는다. **같은 값을 인사이트(`GET /skin-insights`)도 쓴다** — 가르는 기준은 "어느 기능인가"가 아니라 **"사용자가 지금 빈 화면을 보고 있는가"** 이기 때문이다. 분석 세 경로는 POST 이고 앱이 "분석 중" 단계를 그리며 기다린다.

> 두 조회 호출의 실패 결과는 다르다 — 주간 코멘트는 서비스가 삼켜 문장만 빠지고, 인사이트는 저장하지 않고 던져서 다음 조회가 재시도가 된다(§18.10 의 1회 고정 정책). 어느 쪽이든 **오래 기다린 끝의 실패가 빨리 온 실패보다 나쁘다**는 것은 같다.
>
> 12 는 §17.2 가 적은 AI 응답 5~8초 위로 여유를 둔 값이다. **더 조이기 전에 로그를 본다** — 이 경로의 실패는 조용해서(문장만 사라지고 화면은 멀쩡하다) 기능이 없어진 것을 아무도 모른다. 같은 이유로 **429 재시도는 이 경로에서 사실상 죽는다**(전체 마감 14초 중 재시도 대기가 2초를 먹는다). 알고 두는 것이다 — 재시도를 살리자고 예산을 늘리면 문장 하나를 위해 조회 화면을 30초 세우게 된다.
...
| **6** | 음식 API, **표준 영양 테이블 확정** | ML Kit 마무리 + 크롭 · **컷오프 판단** | S07 골격 | 자막·나레이션 초안 | 배포 서버에서 피부 E2E |
...
| R18 | **AI 영양 추정치 변동으로 점수가 흔들림** | 재현성 주장 붕괴 | 높 | 음식명 매칭으로 공공데이터 표준값(식약처 1,452종)을 덮어쓴다. 화면에 "표준 영양 DB 기준" 라벨 표기 |
...
| "같은 사진 두 번 찍으면 같은 점수 나옵니까?" | 룰 엔진은 결정론적이다. 다만 영양값 추정은 AI가 하므로 흔들릴 수 있어, **음식명이 잡히면 식약처 공공데이터 1,452종의 표준값을 사용**한다(화면에 라벨 표기). AI는 "무슨 음식인가"만 판단한다 |
...
app:
  ...
  ai:
    ...
    view-timeout-seconds: ${VIEW_TIMEOUT_SECONDS:12}  # 주간 코멘트·인사이트. 상한 15 — 조회 화면이 멎는 시간이다
    mock: ${AI_MOCK:false}
...
| 재현성 | 음식명이 잡히면 공공데이터 표준값(1,452종)으로 덮어쓴다. AI는 "무슨 음식인가"만 판단 |
