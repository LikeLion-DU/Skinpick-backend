...
- 타임아웃은 재시도 없는 단발. 음식·문장 25초 / **피부 28초**(상한 — 429 재시도가 끼면 최악 30.1초라 클라이언트 32초 안에 들어와야 한다) / **리포트 문장 8초**(조회 화면이라 짧게 — 실패해도 숫자는 그대로 나간다). 429만 1회 재시도, `TimeoutException`은 `AI_TIMEOUT`으로 분기
