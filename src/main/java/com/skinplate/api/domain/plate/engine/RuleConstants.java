...
public final class RuleConstants {
     ...
     * 80 후반~90 대에 닿게 했다.
     *
     * <p><b>바닥도 같이 +5 올라갔다.</b> BASE 는 모든 접시에 똑같이 더해지는 오프셋이라
     * 분포를 넓히지 못한다 — 실제 폭은 가점 델타가 만든다. 실측(2026-08-20): 탄탄면
     * 53→58 · 감자튀김 58→63 · 돈가스_치즈 57→62. "나쁜 한 끼는 제자리"가 아니다.
     */
    ...
    // 이 룰표로 증명할 수 없다.
    //
    // ⚠️ 다만 97 은 "최고의 한 끼 하나가 닿는 점"이 아니라 **평탄면**이다 —
    // 실측(2026-08-20 · 시연 지표): 1,443종 중 92종(6.4%)이 97 에 몰린다.
    // 그 구간에서는 (1) 서로 다른 음식이 같은 점수로 보이고 (2) 원점수가 상한을 넘어
    // **기본+델타 합이 총점과 어긋나며** (3) 행동 시뮬레이션이 0점 움직인다
    // (예시 B: 원점수 107 → 97, R04 카드가 +4 를 광고해도 97→97).
